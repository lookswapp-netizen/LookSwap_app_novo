...
type IconProps = {
  // FIX: Added 'x-mark' to the IconProps type to allow its usage in components like DeployModal.
  name: 'upload' | 'check' | 'sparkles' | 'download' | 'reset' | 'wand' | 'clock' | 'x-mark';
  className?: string;
};
...
export const Icon: React.FC<IconProps> = ({ name, className = 'w-6 h-6' }) => {
  const icons: Record<string, React.ReactElement> = {
    ...
    wand: (
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" d="M9.75 3.104v5.714a2.25 2.25 0 01-.659 1.591L5 14.5M9.75 3.104c.251.023.501.05.75.082a9.75 9.75 0 017.5 7.5c.036.249.07.498.098.746M9.75 3.104C6.19 2.997 2.997 6.19 2.997 9.75c0 .252.028.502.082.75L5 14.5M15 15l6-6m0 0l-6-6m6 6H9" />
        </svg>
    ),
    clock: (
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
    ...
    // FIX: Added the SVG definition for the 'x-mark' icon.
    'x-mark': (
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
